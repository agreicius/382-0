...
},
{
  ...
  "body": " Holomorphic implies analytic   Let be holomorphic on the open set . Given any and satisfying , we have a convergent power series representation for all . Furthermore, given any satisfying , we have , where , .  In particular, is holomorphic at if and only if is analytic at . Using logical shorthand: .    Given we can find an such that . (See .) Since is analytic on , we have , where , . Since for all , we have , and hence for all . We then have , where . Of course, there is one subtle detail to consider here: namely, why are we justified in swapping the order of the integral and infinite sum? The answer, as usual, hinges on a uniformly convergent sequence. In this case, letting . We claim that uniformly on . Indeed, since is continuous on , we have (using the extreme value theorem) for some . Furthermore, since power series satisfy a uniform convergence property, we know that uniformly. Given any , we can thus pick an integer such that for all and . It follows that for all and . This proves our uniform convergence claim, and thus justifies our bringing the integral into the infinite sum.  Lastly, we observe that the derivative formula for the power series coefficients follows from . As for the stated integral formula for , by the deformation of paths prinicple, we have for any  , where , . This completes the proof.   "
},
{
...
},
{
  ...
  "number": "1.19.3",
  "title": "Order of a zero.",
  "body": " Order of a zero   Compute for the given and .    , .     , .          Let . Using formula we see that . Since and , we see that .    The function has power series representation . It follows from that for and . We conclude that .      "
},
{
  "id": "cor_order_zero",
  "level": "2",
...
},
{
  ...
  "number": "1.19.5",
  "title": "Order of <span class=\"process-math\">\\(f\\)<\/span> at <span class=\"process-math\">\\(z_0\\)<\/span>.",
  "body": " Order of at   Let .   Compute .    Find an analytic function such that .          We have .    We see from the power series expansion that .      "
},
{
...
},
{
  ...
  "body": " Different Laurent series   Let . Compute a convergent Laurent series for centered at in the given annulus                      We first use partial fractions to write . Each computation below then boils down to deriving a Laurent series expansion of and in the given region.   If then and . We maneuver into a place where we can use the geometric series for both fractions: We conclude that . It should come as no surprise that our Laurent series expansion in this case is a power series, as is in fact analytic on the entire ball .    For satisfying , we may continue to use our power series expansion for the second fraction, but not the first, since now . But of course, this is equivalent to . So we maneuver accordingly: Thus .    Finally, for , we have and , yielding (similarly as above) and thus .      "
},
{
  "id": "s_laurent_series-10",
  "level": "2",
  "url": "s_laurent_series.html#s_laurent_series-10",
  "type": "Remark",
  "number": "1.20.7",
  "title": "Multiplying power series.",
  "body": " Multiplying power series  The next example requires multiplying together two power series. In general, given two power series that converge for all , their product has power series expansion , where .  "
},
{
  ...
  "body": " Truncated Laurent series   Let be the annulus . Compute the Laurent series of in up through .    We have    "
},
{
  "id": "s_laurent_series-12",
  "level": "2",
  "url": "s_laurent_series.html#s_laurent_series-12",
  "type": "Remark",
  "number": "1.20.9",
  "title": "Expanding a polynomial about a different center.",
  "body": " Expanding a polynomial about a different center  Since a polynomial is entire, we can expand it as a power series about any center . We see using formula that , since is the zero function for all .  "
},
{
  ...
  "body": " Laurent series of rational function   Let be the annulus : , . Compute the Laurent series of on .    Let . We first expand about as . Now compute .   "
},
{
  "id": "s_cauchy_residue",
  "level": "1",
...
},
{
  ...
  "body": " Trig integral over   Compute .    We have Let be the definite integral we wish to compute, let , and let Using the quadratic formula, the roots of denominator are Looking at the moduli of and , we see that and . (In more detail, .) and the Cauchy residue formula then implies .   "
},
{
  "id": "fig_semicircle",
  "level": "2",
...
},
{
  ...
  "body": " Integral of power function   Compute .    Let , where as usual , and is the branch of defined on taking values in . The function is analytic everywhere on except for the roots and of the denominator, which are simple poles of . We compute . For all , and and satisfying , let be the path as in . By the Cauchy residue theorem we have . Yes, we resorted to some algebraic trickery there (factoring out the ) in order to combine terms nicely. This was not strictly speaking necessary, but saves us some simplifying work at the end.  We now compute the integrals along the various subpaths that make up . Using straight line parametrizations for the integrals along and we see that , where in our computation of , we use the fact that . Taking the limit as we get . Once again, the fact that the limit can be brought into the first two integrals relies on the convergence of the integrands to being uniform for all . The fact that can be shown using more elementary means.  Next, the usual ML-inequality arguments suffice to show that as and as . Thus taking limits as and , in our last equality, we have . We conclude that . That is a fine answer. If, however, you insist on not having it expressed in terms of sine, we can use a trig identity on  , and then conclude .   "
},
{
  "id": "s_winding",
  "level": "1",
  ...
  "body": " Winding number  Reminder: unless stated otherwise, paths are assumed to be piecewise smooth.   Winding number   Let be a closed path, and let be a complex number not lying on . The winding number (or index ) of around (or with respect to) , denoted is defined as .     Winding numbers of circles   Fix a positive integer . Given any and , let , . Prove: . In other words, in this special case, computes the number of times the path  wraps around the point .    After a re-parameterization, we can write , where , : in other words, our -fold trip around the circle is a concatenation of simple trips around the circle. But then we have .     Winding number properties   Let be a closed path and let .    is an integer for all .    The function is continuous. As a result, is constant on all open connected subsets of .            Interior and exterior of path   Let be a closed path and let . The interior and exterior of , denoted and , respectively, are defined as follows: .     The term interior is a potential source of confusion, as we defined earlier the notion of the interior of a subset of ( ). Observe that our new notion of interior applies not to a set , but rather a path , which is a function.    Properties of and    Let be a closed path.   If is an elementary region and , then .     is a bounded set.     is an unbounded set.       We first prove statement (1). Assume , with elementary. For any , the function is analytic on . By the definition of an elementary region, it follows that . Thus if , then . It follows that .  Statements (2) and (3) follow from (1) and the observation that for all (by the extreme value theorem), and hence . Since is elementary, we have and . It follows that is bounded and is unbounded.     Winding number residue theorem   Let be an elementary region, and let be analytic on , where for all , is an isolated singularity of and for . Given any closed path lying in , we have .    For each we have a Laurent expansion . We double-index the coefficients here as they depend also on . For each , let , the so-called principal part of the Laurent expansion of at . It follows from the definition of convergence of Laurent series, that since the series expansion converges for all , the principal part converges for all . It follows extends to an analytic function on , and further that the function is analytic on except at each , where it has removable singularities. The function can thus be extended to an analytic function on . Since is elementary and is closed, we have . Lastly, since for all , for all integers , and for all (use an antiderivative!), we have , as desired.     Argument principle   Assume is an isolated singularity of of nonzero order .    .     .       Write where , is analytic at and . We then have and thus . Since , the reciprocal is also analytic at , and thus so is . It follows that , whence .     Meromorphic   Let be an open subset of . A complex function is meromorphic on if for all , is either analytic at , or is a pole of finite order of .     Argument principle   Let be an elementary region, and assume is meromorphic on . Let be a closed path in that does not contain any zeros or poles of .   Argument principle  We have . In other words, the logarithmic derivate integral on the left side of is equal to the winding number of the path around .    Zeros and poles theorem  Let be the set of zeros of in , and let be the set of poles of in . We have . In particular, if for all , we have In this special case, we summarize as , where our count includes multiplicities : , we take into account the order of each zero and pole of .          Let be our closed path. We have .    First observe that since is elementary, we have . Since is meromorphic, it follows that has only finitely many zeros and poles in . (See .) Thus we can apply the Cauchy residue theorem to conclude , where we have used for all poles and zeros of .        If is meromorphic on an elementary region , and if is a path in that avoids all zeros and poles of , then it is not difficult to show that . One can further show (this requires more analysis) that has a finite number of zeros in poles in the interior of . This means that the sums in and are in fact finite.   "
},
{
  "id": "d_winding_number",
  "level": "2",
...
},
{
  ...
  "body": " Winding numbers of circles   Fix a positive integer . Given any and , let , . Prove: . In other words, in this special case, computes the number of times the path  wraps around the point .    After a re-parameterization, we can write , where , : in other words, our -fold trip around the circle is a concatenation of simple trips around the circle. But then we have .   "
},
{
...
},
{
  ...
  "type": "Corollary",
  "number": "1.24.6",
  "title": "Properties of <span class=\"process-math\">\\(\\Int \\gamma\\)<\/span> and <span class=\"process-math\">\\(\\Ext \\gamma\\)<\/span>.",
  "body": " Properties of and    Let be a closed path.   If is an elementary region and , then .     is a bounded set.     is an unbounded set.       We first prove statement (1). Assume , with elementary. For any , the function is analytic on . By the definition of an elementary region, it follows that . Thus if , then . It follows that .  Statements (2) and (3) follow from (1) and the observation that for all (by the extreme value theorem), and hence . Since is elementary, we have and . It follows that is bounded and is unbounded.   "
},
{
  ...
  "body": " Winding number residue theorem   Let be an elementary region, and let be analytic on , where for all , is an isolated singularity of and for . Given any closed path lying in , we have .    For each we have a Laurent expansion . We double-index the coefficients here as they depend also on . For each , let , the so-called principal part of the Laurent expansion of at . It follows from the definition of convergence of Laurent series, that since the series expansion converges for all , the principal part converges for all . It follows extends to an analytic function on , and further that the function is analytic on except at each , where it has removable singularities. The function can thus be extended to an analytic function on . Since is elementary and is closed, we have . Lastly, since for all , for all integers , and for all (use an antiderivative!), we have , as desired.   "
},
{
  "id": "lem_arg_princ",
  "level": "2",
...
},
{
  ...
  "body": " Rouché's theorem   Let and be analytic functions on the elementary region , and let be a closed path in elementary region, let be a closed path such that for all . If for all , then and have the same number of zeros in (counting multiplicity).    Proof A  Observe first that the condition for all implies that and for all .  Let and . We wish to show that . This is the precise way of stating that the number of zeros of in is equal to the number of zeros of in (counting multipilicty).  Since is nonzero on we have on . Letting , we have . The penultimate equality is an instance of the more general property , which is essentially a consequence of the product rule for derivatives. (We leave this elementary proof to the reader.)  Lastly, observe that since for all , we have . Since is an elementary region, it follows from that , and hence that . We conclude that , as desired.    Proof B (sketch)  As an alternative, somewhat slicker proof, consider the function defined as . First show that is continuous, then conclude that is constant, since is connected and is discrete. It follows that , as desired.   "
},
{
  "id": "eg_rouches_thm",
  "level": "2",
...
},
{
  ...
  "number": "1.27.6",
  "title": "Conformal maps.",
  "body": " Conformal maps   Let be an open subset of , and let be a real differentiable map. Making our usual identification of with , we treat as a complex function.   For all , is conformal at if and only if is analytic at and .    Given an open subset , is conformal on if and only if is analytic on and for all .     is conformal and injective on if and only if is biholomorphic.       The key to all the proofs is the fact that a real differentiable function , , is complex differentiable if and only if and satisfy the Cauchy-Riemann equations at all .   "
},
{
  ...
  "body": " Conformal map: squaring function   Let ,   Show that is conformal on     Illustrate with an explicit geometric example that does not preserve the angle between curves intersecting at .          Clearly is analytic everywhere, and is nonzero at all . Thus is conformal on by .    Consider the rays and , which intersect at at an angle of . However, their images under are and , which intersect at an angle of .      "
},
{
  "id": "d_mobius_trans",
  "level": "2",
